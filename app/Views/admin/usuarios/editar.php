    ...
    <div class="container">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Editar #<?= $usuario['id'] ?></li>
            </ol>
        </nav>
            ...
            <div class="row align-items-center">
                <div class="col-md-8">
                    ...
                    <p class="mb-0 opacity-90">
                        <i class="fas fa-envelope me-2"></i><?= esc($usuario['email']) ?>
                        <span class="ms-3">
                            <i class="fas fa-calendar me-2"></i>Registrado: <?= date('d/m/Y', strtotime($usuario['fecha_registro'])) ?>
                        </span>
                    </p>
                </div>
        ...
        <div class="admin-card">
            <form id="formUsuario">
                ...
                <div class="mb-3">
                    ...
                    <div class="form-text">Déjalo vacío para mantener la contraseña actual. Mínimo 6 caracteres si deseas cambiarla.</div>
                    <div class="invalid-feedback"></div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="ciudad" name="ciudad" value="<?= esc($usuario['ciudad'] ?? '') ?>">
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="codigo_postal" name="codigo_postal" value="<?= esc($usuario['codigo_postal'] ?? '') ?>">
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
                    <div class="col-md-6">
...
<script>
    ...
    const usuarioRoles = <?= json_encode($usuario['roles'] ?? []) ?>;
            ...
            .then(response => response.json()).then(data => {
                if (data.success && data.data) {
                    ...
                    data.data.forEach(rol => {
                        const isSelected = usuarioRoles.some(userRol => userRol.nombre === rol.nombre);
                        select.innerHTML += `<option value="${rol.id}" ${isSelected ? 'selected' : ''}>${rol.nombre}</option>`;
                    });
                }
            })
    ...
    function handleUserForm(event) {
        ...
        fetch(`<?= base_url('admin/api/usuario') ?>/${usuarioId}`, {
                method: 'POST',
                headers: {
